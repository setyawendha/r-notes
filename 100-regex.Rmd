...
```{r xkcd-regular-expressions, fig.pos='!h', echo=F, fig.cap="Regular Expressions, by xkcd.", out.width="60%", fig.align="center"}
...
Suppose instead that we are interested in counting occurrences, in some arbitrary given string, of any of the following three strings in some arbitrary given string:
...
```{r}
# function to count occurrence of substrings in string.
...
```{r character-class-names, echo=F}
...
* `(a\d{2,})+` matches one or more occurrence of a followed by at least two digits.  Thus it matches "a23" and "a23a773".  It won't match "a2a3".  It will match only the "a23" in "a23a6" and it will match only the "a567" in "a4a567".
...
Suppose that you have a string containing a number of words involving "bed", and you would like to find all of the instances in which "bed", except for the word "bedtime".  With the tools we have so far this is a difficult task.  Fortunately there are *look-aheads* \index{look-ahead} to simplify our work.
...
* It concludes the a look-ahead group.  The parentheses mark out the group.  The initial `?` indicates that we plan to look ahead. The `!` may be thought of as "not equals"; it means that if we find the pattern that follows the `!` we will not have a match.
...
```{r regex-string-escape-examples, echo=F}
...
It seems that the folks who entered the dates were not consistent in how to format them.  In order to make analysis easier, it wold be better if all the dates had exactly the same format.  With the function `gsub()` \index{R-functions!gsub()@\texttt{gsub()}}and regular expressions, this is not difficult:
...
* It can include special tokens for case-conversion:
    * `\U` converts the rest of the replacement to upper-case (or ceases conversion when `\E` or `\L` occurs).
    * `\L` converts the rest of the replacement to lower-case (or ceases conversion when `\E` or `\U` occurs).
...
capVowels <- function(str) {
  gsub(pattern = "([aeiou])", replacement = "\\U\\1\\E", x = str, perl = T)
}
...
starRepeats <- function(str) {
  gsub(pattern = "(\\b(\\w+) \\2\\b)", replacement = "*\\1*", x = str, perl = T)
}
...
```{r}
grep(pattern = "\\bT\\w*\\b", x = sentences, perl = T)
```
...
```{r}
grepl(pattern = "\\bT\\w*\\b", x = sentences, perl = T)
```
...
```{r}
hasTWord <- grepl(pattern = "\\bT\\w*\\b", x = sentences, perl = T)
sentences[hasTWord]
```
...
```{r}
matchInfo <- gregexpr(pattern = "\\bT\\w*\\b", text = sentences, perl = T)
str(matchInfo)
```
...
```{r anchor-links, cache = T}
links <- list()
for (i in 1:nrow(hunger)) {
  text <- reviews$content[i]
  m <- gregexpr(linkPattern, text = text, perl=T)
...
```{r tabulated-link-numbers, echo=F}
...
findMister <- function(str) {
  #mister <- "((\\w+)\\b)(?<=Mr\\.\\b)"
  mister <- "(?<=Mister )\\w+\\b"
  results <- gregexpr(pattern = mister, text = str, perl = T)
...
findTitled <- function(str) {
  ...
  results <- gregexpr(pattern, text = str, perl = T)
  unlist(regmatches(x = str, m = results))
}
capRepeats <- function(str) {
  gsub(pattern = "(\\b(\\w+)\\s+\\2\\b)", replacement = "\\U\\1\\E", x = str, perl = T)
}
longWord <- function(strs) {
  indices <- grep("\\b\\w{8,}\\b", strs, perl = T)
  strs[indices]
}
longWord2 <- function(strs) {
  results <- gregexpr("\\b\\w{8,}\\b", strs, perl = T)
...
1.  For each of the following expressions, write a regular expression to test whether any of the sub-string(s) described occur in a given string.  The regular expression should match any of the sub-strings described, and should not match any other sub-string.  Try to make the regular expression as short as possible.  Write the regular expression as a string that could be used in one of R's regex functions (i.e. extra backslash escapes as needed).  The first item is done for you, as an example.
    ...
    * *slick* ans *sick*
    * Any word ending in *ity* (such as *velocity* and *ferocity*).  Be sure to pay attention to word-boundaries.  You should match *velocity* but not * velocity* or *velocity;*.
