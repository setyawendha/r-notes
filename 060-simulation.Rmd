...
```{r monkey-typewriter, echo=F, fig.pos='!h', fig.cap="A monkey typing randomly manages to type out a line of Shakespeare.  What is the chance that the monky would produce this particular line?  Source: http://www.gloryofkings.com/?p=189.", out.width="40%", fig.align="center"}
...
```{r}
set.seed(4040)
sims <- sample(tickets, size = 10000, replace = T)
prop.table(table(sims))
```
...
x <- c(-1, 0, 2)
px <- c(0.25, 0.50, 0.25)
winnings <- sample(x, size = 10, replace = T, prob = px)
winnings
```
...
```{r echo=2:3}
set.seed(4545)
winnings <- sample(x, size = 10000, replace = T, prob = px)
mean(winnings)
```
...
When we ask R to add the elements of a logical vector, it coerces the Boolean values to number:  `TRUE` becomes 1 and `FALSE` becomes 0.  Summing the numbers is then equivalent to counting how many times `TRUE` appeared in `triangle`.  Taking the mean is equivalent to computing the proportion of `TRUE`s in `triangle`.
...
We would like to force R to provide a seed when it is not given one to use.  Such a seed should vary from one call to another in a way that cannot be predicted easily. 

I suggest the use of the `Sys.time()` function\index{R-functions!Sys.time()@\texttt{Sys.time()}}, which gives your computer's idea of the current time:
...
The result is---on Unix-like computer systems---the number of seconds elapsed since January 1, 1970.  It should serve pretty well as a seed.  The following code implements a `seed` parameter in such a way that the system-time is used when the user does not supply a seed.

```{r}
triangleSim <- function(reps = 10000, table = FALSE, seed = NULL) {
  # if user does not provide a seed, set one
...
As long as you don't make the very same simulation-function call twice in the same second, you should get different results on different calls.
...
The above method of program development is called *encapsulation and genralization*.  For small projects of the sort that we have on hand, it's a good way to go about writing a computer program. \index{encapsulation and generalization}
...
We will need to run the `tringleSim()` function several times, with a different choice for `reps` each time, keeping track of the estimates we obtain each time.  In the process we won't need tables or other output to the console.  Let's rewrite `triangleSim()` for maximum flexibility, making console output optional and using the invisible return discussed in Section \@ref(invisible-returns).^[The addition of the invisible return may be thought of as yet another step in the encapsulation-and-generalization process of program development!]
...
If you want to go through the above process 20 times instead of just doing it once, you could ask for:
...
Nothing in `rbinom()` says that it addresses only with coin-flipping.  In general it serves as a model for any situation in which:
...
* the chance of the Success outcome is the same, on any trial (e.g., the coin always has the same chance of coming up heads, etc.)
...
Suppose we were to watch Judge A making 20 decisions, recording each time whether he was correct or not.  In order to simulate the results with R, we could ask for:
...
This would give us a *count* of the number of times Judge A was correct, but it would not tell us which of the cases he judged correctly and which he judged incorrectly.
...
```{r courtresults1, cache = T}
courtSim(reps = 100000, seed = 3838, table = T)
```
...
```{r}
courtSim2(reps = 100000, seed = 3838, table = T)
```
...
* After we finished all of the repetitions, we computed the proportion of those repetition in which the event *did* occur.
* This proportion served as our estimate of the probability for the event to occur.  The more.
...
```{r unifsum1000, echo = 2:7, cache = T}
set.seed(5050)
needed <- numeric(1000)
for (i in 1:1000 ) {
...
Now that we have the basic idea of how to accomplish the simulation, it remains to package up our code into a nice function that will enable a user to set the number of repetitions, set a seed, and choose reporting options.  Along the way we will generalize a bit, allowing the user to set a target sum other than 1.
...
```{r expSumFinal, cache = T}
numberNeededSim(target = 1, reps = 10000, seed = 4848, table = TRUE)
```
...
I would also like to get some idea of the distribution of $X$---the chance for it to assume each of its possible values.  Unlike the previous example, however, $X$ is what we call a *continuous* random variable:  \index{continuous random variable}it can assume any of the values in a *range of real numbers*.  After all, the amount left in the "other" box could be anything from 0 to 150 feet:  it could be 3.5633 fee or 5.21887 feet, or any real number at all, so long as it's between 0 and 150.  Although any real number value---3.5633 feet, for instance---is *possible*, it seems that the chance of $X$ being *exactly* 3.5633 is vanishingly small, and the same goes for any of the possible real numbers values for $X$.  Hence it seems that the distribution of $X$ cannot be described well with a table of the values that we actually get in a simulation:  indeed, we are highly unlikely to get any value more than once in the course of our simulation.
...
We have just asked R to produce 100 numbers chosen randomly from a *normal* distribution.  The `mean` parameter sets what the values should be "on average", and the `sd` parameter indicates how much they should typically differ from that average value.  The bigger the `sd`, the more the likely difference.
...
```{r dentalflosssim, cache=T, width.out="60%", fig.cap="Density plot of the results of a dental-floss simulation.", fig.align="center"}
flossSim(reps = 10000, graph = TRUE, seed = 3535)
```
...
```{r right-triangle, echo=F, fig.cap="Sides of a right triangle with hypotenuse 1, in terms of the angle shown.", out.width="75%", fig.align="center"}
...
```{r two-steps, echo=F, fig.cap="The first two steps by the Drunken Turtle.  Turtle turns by $a_1$ radians for Step One, then by $a_2$ radians for Step Two.", out.width="75%", fig.align="center"}
...
We need the $x$-coordinates after all of the previous steps, as well.  This can be accomplished by R's `cumsum()` function. \index{R-functions!cumsum()@\texttt{cumsum()}} You may think of `cumsum` as short for "cumulative sum".  Given a vector of `vec` numbers, `cumsum(vec)` returns a vector of the same length, where for every index `i`, `cumsum(vec)[i]` is `sum(vec)[i]`.   This is made clear in the following example:

```{r}
vec <- c(1, 1, 1, 1, 1)
cumsum(vec)
```
Accordingly, we can get the $x$-coordinates of the first ten steps like this:
...
1.  Consider the following two games.
    ...
    * In Game B you roll a fair die. If the six-spot comes up, you win twenty dollars.  If you get 3, 4, or 5, nothing happens.  If the one-spot comes up, you lose fifteen dollars.
    
    You can choose either to play Game A ten thousand times or to play Game B ten thousand times.  You wonder:  *which game is the better choice?*  Before you answer the question, write two simulation functions:  one that provides an estimate of the expected value of the winnings from a play of Game A, and another that estimates the expected value of the winnings from a play of Game B.  The functions should have two parameters:
    * a `rep` parameter to permit the user to choose the number of repetitions of the game;
    * a `seed` parameter to permit the user to choose a starting seed.
    
    Try both functions with ten thousand reps and with simple starting seeds.  Report the seeds you used for each function, and the estimated expected values for each game.  Use your results to decide which game is the better choice.
...
1.  Refer back to the `numberNeededSim()` function.  Modify it so that it returns the estimated expected value invisibly, and has an option to turn the `cat()` report to the console.  Write a program that computes estimated expected number-needed for the following targets:
