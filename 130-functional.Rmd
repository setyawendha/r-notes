...
Observe also that each element of the returned list `numberNA` was an integer vector of length 1.  Hence no real purpose was served by the complex list structure:  the information could have just as well been conveyed as a integer vector.
...
The next example illustrates the use of the return-value of `walk()`.  We would like to save plots of all numerical from the data frame `m111survey`, and also print summaries of them to the Console.
...
1. Use `map_dbl()` to produce the squares of the whole numbers form 1 to 10.

2.  Use `map_chr` to state the type of each element of the following list:
...
makeTable(1)  # all logical vectors of length 3
```
...
The last two of them begin with `FALSE` and *also* end with the vectors of $S_2$:
