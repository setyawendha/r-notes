...
```{r ggboxplotsample, echo=FALSE, cache=TRUE, fig.cap="Illustration of a simple box plot."}
...
ggplot(m111survey, aes(x = fastest, y = GPA)) +
  labs(x = "fastest speed ever driven (mph)",
       ...
       subtitle = 'A title should briefly indicate the main lesson the plot has for us.  Instead of: "Scatterplot of\nGPA vs. fastest speed", say something like: "Speed and GPA are not strongly related."')
```
...
Some people don't like the glyphs "stacked" in bar graphs.  In order to mollify them we can set `position` to "dodge", as in the code below.  The results appear in Figure \@ref(fig:sexseat2).
...
If you want to add another layer of glyphs, simply add on another call to a `geom`-function.  In order to produce Figure \@ref(fig:violinfastestseat), for example, we use the code below:
...
Infromation about the season in which the day occurs is spread over the three 0-1 numerical variables `spring`, `summer` and `fall`.  We propose to construct a single factor variable `season` variable from these three variables, as follows: 
...
```{r echo =FALSE, cache=TRUE, birthsplotframes, fig.cap = "Some of the days have significantly fewer births.  What's going on?",fig.align="center", out.width = "60%"}
...
```{r violinbirthswday, cache=TRUE, echo=TRUE, fig.cap = "Violin plot of births, by day of the week.",fig.align="center", out.width = "60%"}
...
```{r birthsplotframesweekend, echo =TRUE, cache=TRUE, fig.cap = "The days with fewer births are almost always weekend-days.",fig.align="center", out.width = "60%"}
...
1.  Using the `tigerstats::m111survey` data frame, write the **ggplot2** code necessary to produce the graph in Figure \@ref(fig:heightideal).  (The points are all blue.)
...
1.  Using the `mosaicData::Utilities` data frame, write the **ggplot2** code necessary to produce the graph in Figure \@ref(fig:utilitybills).

    ```{r utitlitybills, cache=TRUE,echo=FALSE, fig.pos='!h'}
    monthAbbr <- with(Utilities, 
                      plyr::mapvalues(month, from = 1:12,
                                      to = month.abb))
    monthAbbr <- factor(monthAbbr, levels = month.abb)
...
1. Look at the plot you made in the previous exercise:  you will notice that one person made a wage that was much higher than all the rest.  In data analysis, when a value is much higher or lower than the rest of the values we call it an *outlier*.
    ...
    Create a new data frame called `cpsSmall` that is the same as `CPC85` except that it excludes the row corresponding to the outlier-individual.
