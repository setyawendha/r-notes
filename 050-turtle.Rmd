...
By default the turtle is positioned in the middle of a square of dimensions 100 unit by 100 units.  (These dimensions can be changed.)  You can get the turtle's position at any time:
...
There are many, many colors to choose from, and 657 of them even have names.  To view them, use:
...
Notice that in the above code the turtle was initially set so a bit to the left of center, so that the resulting circle would be situated close to middle of the region.
...
We actually have the ability to set the initial value for ourselves.  This is called *setting the random seed*.  From any specified seed, the result of calls to R's random-data functions will be completely determined (although---just as in the case of "real" randomness---the output will still probably "look" random).
...
1. Write a function called `turtle_gon()` that draw a regular polygon.  The user should be able to specify the side-length and the number of sides.  Start with `turtle_init()` and use your function to draw a regular dodecagon (twelve sides) with each side having a side length of 115 units. Your figure should not stray outside of the turtle's field, so you may have to move your adjust the position of your turtle a bit prior to calling your function.
...
1. Make a new star function `turtle_rstarColors()` that behaves like `turtle_rstar()` except that instead of being determined by the user the ray-color varies randomly from one way to another, as in Figure \@ref(fig:turtle-star-3): 
