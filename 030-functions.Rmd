...
`fucntionName` is the variable that will refer to the function you define.  Like any other variable, it can contain letters, numbers, the dot and the underscore character, but is not permitted to begin with the dot followed by a number.

After the `function` reserved word we see a pair of matching parentheses.  The contain the *parameters*\index{parameter} of the function, which will be passed into the function as variables referred to by the parameter names.

The *body* \index{body of a function} of the function consists of one or more expressions that do the work of the function.  Note that the body is enclosed with curly braces.  This is only necessary, tough, if the body consists of more than one expression. If the body had only one expression then that expression could appear without the braces, like this:
...
You will have noticed by now that parentheses are essential when using a function. What would happen if we asked typed just the function's name itself rather than trying in a call?  Give it a try:
...
Sometime around the year 1400CE, the South Indian mathematician Madhava discovered toe following infinite-series formula for $\pi$, the ratio of the circumference to the diameter of a circle:
...
* The $k = \infty$ above the sum-sign says that we are to keep on going like this, increasing $k$ by 1 every times without stopping.
...
* We called the `match.arg()`\index{R-functions!match.arg()@\texttt{match.arg()}} function, which found that element of `inhabitants` that matched what the user actually submitted for the parameter `inhabitant`.  This element was them assigned to `x` and we used `x` to report the desired color.
...
In R an *environment* is particular kind of data structure that helps the computer connect names to a value.  An environment can be thought of as a bag of names---names for vectors, functions, and all sorts of objects---along with a way (provided automatically by the computer) of getting from each name to the value that it represents. The process that the computer follows in order to connect a name to a value is called *scoping*.\index{scoping}  In R, as in many other computer languages, environments are what makes scoping possible.  In other words, environments are how R figures out what the names in any piece of code *mean*.
...
Wait a minute:  why did this work?  Doesn't the Global Environment come before package **base** in the search path?  Yes it does, but since the first occurrence of `cat` was followed by an open parenthesis R know to expect that it referred to a function.  Hence it kept looking up the search path for a function with the name `cat`, eventually finding our familiar `cat()` function in **base**.
...
If I want to keep my `cat()` and still use the **base** version of`cat()` as well, I can do that to be sure of getting a particular package's version of a function, put the name of the package and then two semicolons before the function-name, like this:
...
* The Global Environment is the environment that is active when we are working form the console.
...
Just as the Global Environment is a child of the last package loaded, so the Global Environment can have children of its own.  In fact a child-environment is created whenever we define a function in the Global Environment.
...
: An argument for a function is a value that is assigned to one the parameters of the function.  (Sometimes arguments are called an *actual arguments* in order to distinguish then from parameters that are often called *formal arguments*.)
...
:  A special envrionment that is created when a fuction is called and ceases to exist when the function finishes executing.  It contains the values that are local to the function and the arguments of the function as well.
...
1.  A vector is said to be a *palindrome* if reversing its elements yields the same vector.  Thus, `c(3,1,3)` is a palindrome, but `c(3,1,4)` is not a palindrome.
    ...
    ```{r}
    palindrome(vec = c("Bob", "Marley", "Bob"))
    ```
    
    ```{r}
    palindrome(c(3,2,7,4,3))
    ```
...
1.  The eighteenth-century mathematician Leonhard Euler discovered that:
    ...
    Use this fact to write a function called `eulerPI()` that will approximate $\pi$.  The function should take a single parameter `n`, which is the number of terms in the infinite seres that are to be summed to make the approximation.  The default value of `n` should be 10,000.
