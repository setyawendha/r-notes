...
Next, re-make the `manyCat()` function, but to make it a bit more interesting, give the parameter `n` a default value of 3.
...
* R sees that you want to assign the value `"Hello!"` to the parameter `word`.
* R sees that you want assign the value 4 to the parameter `n`.
* "Great!", says R.  "I've got everything I need to run this function."
...
* "That's OK", says R. "I'll use the default value of 3 for `n`."
* R executes the code in the body of the function.

Next, call the function as follows:

```{r}
manyCat(word = "Hello!", n = n)
```
...
* "Fine", says R. "I'll make sure that I can find this `n` thingie before I run the function."
* R goes along the search path, finding `n` at once in the Global Environment.
* "Great!", says R.  "Now I know to assign 2 to the parameter `n`"
* R then executes the code in the body of the function.

Now call the function as follows:

```{r}
manyCat(word = "Hello!", n)
```

Again let's consider how this works:

* R sees that you want to assign the value `"Hello!"` to the parameter `word`.
* R sees the `n`.  Since `word` is already assigned, R figures that you mean to assign the value of `n` (in some environment) to the parameter `n`.
* "Fine", says R. "I'll make sure that I can find this `n` thingie before I run the function."
* R goes along the search path, finding `n` at once in the Global Environment.
* "Great!", says R.  "Now I know to assign 2 to the parameter `n`"
* R then executes the code in the body of the function.
...
The call `manyCat(word = "Hello!", n)` will elicit the same error message, for the same reason.

The moral of the story is that parameters really, really are NOT the same thing as arguments, even when they are called by the same name.
