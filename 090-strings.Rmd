...
for ( i in 1:length(splitDates) ) {
  ...
  # character vector consiting of the full names of the months of
  # the year:
...
Joining appears to be the opposite of splitting, but in R that's not quite so.  Suppose, for instance, that have dates where the month, day and year are separated by hyphens and you want to replace the hyphens with forward slashes:
...
1. Write a function called `subStrings()` that returns a vector of the substrings of a given string that have at least a given number of characters.  The function should take two arguments:
    ...
    * `n`: the minimum number of characters a substring should have in order to be includes in the vector.
    ...
    Validate the input:  if the argument for `n` is less than 1 or greater than the number of characters in `str`, then the function should advise the user and cease execution.  Typical examples of use should look like this (although it is OK if your vector contains the sub-strings in a different order):
...
2.  Write a function called `subPalindrome()` that, for any given string and specified number $n$, returns a character vector of all the substrings of the strings having at least $n$ characters that are also palindromes.  The function should take two arguments:
    ...
    * `n`: the minimum number of characters a substring should have in order to be includes in the vector.
    ...
    Validate the input:  if the argument for `n` is less than 1 or greater than the number of characters in `str`, then the function should advise the user and cease execution.  Typical examples of use should look like this (although it is OK if your vector contains the palindromes in a different order):
