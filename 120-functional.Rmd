...
Let us begin by exploring the notion of a *programming paradigm* in general. We will go on in this Chapter to consider will two programming paradigms that for which R provides considerable support.  In the next Chapter we will consider a third programming paradigm that exists in R.
...
### Functional Programming:  The Ubiquity of Functions in R
...
The second second slogan indicates that functions are everywhere in R.  It also corresponds to the first principle of the *functional programming* \index{functional programming}paradigm, namely:
...
Functions other than primitive functions are objects of type "closure."

If a function can be a certain type of thing, then it must be a "thing"---an object, things you can manipulate.  For example, you can put functions in a list:
...
This is as we expect:  the variable `a` exists only in the *runtime* environment that is created in the call to `addThree()`.  As soon as the function finishes execution that environment dies, and `a` dies long with it.  In particular, it never became part of the Global Environment.
...
The last principle of the functional programming paradigms that I'm going to sate isn't really a formal principle:  it's really more an indication of the programming style that prevails in languages where functions are first-class objects and that provide other support for functional programming.  The final principle is:
...
With `lapply()` can get the list in our second example as follows:
...
You may recall an [Exericise](#appeals-exercise) in Chapter \@ref(frames) where you were asked to use vectorization to simulate the decisions of judges in the Appeals Court Paradox.  You can `sapply()` to produce the simulated decisions, too:  as follows:
...
means <- sapply(splitSpeeds, FUN = mean, na.rm = T)
means
```
...
```{r}
with(df, tapply(X = fastest, INDEX = seat, FUN = mean, na.rm = T))
```
...
```{r}
with(df, tapply(fastest, INDEX = list(sex, seat), FUN = mean, na.rm = T))
```
...
1. Explain in words what the following line of code produces when given a vector `y`:
    ...
    In the course of our explanation, say whether the result is a vector or a list.
...
1.  Refactor the following code so that it uses `Map()` instead of a loop:
...
1.  To *refactor* \index{refactoring}computer code is to rewrite the code so that is does the same thing, but in a different way.  We might refactor code in order to make it more readable by humans, or to make it perform its task more quickly.
    ...
    Refactor the code for the `randomSims()` so that it uses `lapply()` instead of a loop.
